...
function initCtrlCompPanel(){
  ...
  var $ul = $('<ul class="list" id="id_ctrllist">');
  var $html = null;


  con.append($ul);
  _wndList.Ctrl = new mxWindow('控制系统', con[0], 150, 45, 125, _WIN.height-100, true, true);

// 窗口原始尺寸记录
  _wndList.Ctrl.w=150;
  _wndList.Ctrl.h=_WIN.height-100;
  _wndList.Ctrl.dir=0; //窗口的方向：0正常，1：左，2：右，3：上，4：下


  loadCtrlList(1)
  _wndList.Ctrl.setMinimizable(true);
  _wndList.Ctrl.setScrollable(true);
  _wndList.Ctrl.setVisible(true);
  _wndList.Ctrl.setScrollable(true);


  $('#id_ctrllist').on('mousedown','.component-item',function(e){
...
function insertCtrl(graph_, evt_, target_, x, y){
