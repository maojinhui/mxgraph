...
function showEleProfile(iid,nn,p){
  ...
  for(var i = 0 ; i< _pssEle[nn]['param'].length;i ++){
     html+='<tr><td>'+_pssEle[nn]['param'][i]['label']+'</td><td><input type="text" class="param-inpt" value="'+_pssEle[nn]['param'][i]['value']+'" /></td><td>'+_pssEle[nn]['param'][i]['unit']+'</td></tr>';
  }
  html+='</table>';
  /*
 ...
 /* for( var x in _pssEle[nn]["pin"]){
  ...
  //~ html += '</div><br><button onclick="saveCP(this,\''+iid+'\',\''+nn+'\')">保存</button> <button onclick="cancelCP()">取消</button>';
...
if( _pssEle[nn]['ctrlable'] ==1 )
  ...
  html += '</div><br><br><span onclick="saveCP(this,\''+iid+'\',\''+nn+'\')" class="btn btnsave">保存</span> <span onclick="cancelCP()" class="btn btncancel">取消</span><br><br>';
