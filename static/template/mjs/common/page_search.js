...
PageSearch.prototype = {
    ...
    set_pager : function(total, pagesize, active_page){
        ...
        var obj = this;
		this.pagination.find("a").not(".active").click(function(){
    ...
    send_request : function(conds, page) {
        ...
        $.get(url, conds, function (result) {
            ...
            if (result.total == 0) {
                ...
                obj.set_pager(result.total, pagesize, page);
                obj.add_table_lines(result[obj.settings["results_key"]], result);
            }
...
function draw_center_page(pages, active_page){
    var ul = this.pagination.find("ul.pagination");
    ul.html("");
    ...
    for(var i = 0;i < pages.length;++i){
        ...
        if(i == 0){
            ele = $("<a href='javascript:void(0)' aria-label='Previous'><span aria-hidden='true'>&laquo;</span></a>");
            ele.addClass("sub_one");
        }
        else if(i == pages.length - 1){
            ele = $("<a href='javascript:void(0)' aria-label='Previous'><span aria-hidden='true'>&raquo;</span></a>");
...
/**
 * add_table_lines方法的一种实现，用于在锁/基站/用户列表页面绘制对象列表
 * @this {PageSearch}
 * @param results 后台请求返回的结果列表
 */
function add_fluid_widget(results){
    this.result_div.html("");

    var fluids = [];
    var current_fluid = null;
    for(var i = 0;i < results.length;++i){
        var widget = $("#" + this.settings.id_hidden_widget).clone();
        widget.show();
        this.settings.set_widget(widget, results[i]);
        fluids.push(widget);
    }

    for(var i = 0;i < fluids.length;++i){
        this.result_div.append(fluids[i]);
    }
}
...
function add_row_widget(results){
    ...
    for(var i = 0;i < results.length;++i){
        ...
        if(i % 2 == 1){ //The last, handle next
            this.result_div.append(current_row);
        }
    }
}
